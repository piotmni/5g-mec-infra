...
ANSIBLE_INVENTORY_FILE="../../../ansible/inventory/full/inventory.ini"
...
ANSIBLE_HOST_JSON=$(echo "$TERRAFORM_OUTPUT" | jq -r .ansible_host.value)

# Loop through the JSON object
echo "$ANSIBLE_HOST_JSON" | jq -r 'to_entries[] | [.key, .value] | join(" ")' | while IFS= read -r line; do
